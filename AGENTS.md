...
- Whenever I correct your behavior, establish a new architectural constraint, or express a coding preference (e.g., 'no dependencies'), you MUST immediately persist this rule using whichever persistent-memory mechanism your tool provides.

## Core Architecture

- **Streaming:** Most indicators process data through channels (`<-chan T`), allowing for efficient, pipeline-based calculations.
- **Generics:** Indicators are generic, most over `helper.Float` (float32/float64); a minority that don't need fractional math (e.g. `MovingMax`, `MovingSum`) use the broader `helper.Number` (adds integers).
...
- **Copyright Header:** Every file must start with the copyright notice:
  ...
  ```
- **Indicator Pattern:**
  - Struct named after the indicator (e.g., `Ema[T helper.Float]`).
