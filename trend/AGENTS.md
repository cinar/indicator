...
Indicators follow the standard `Compute(<-chan T) <-chan T` pattern and are generic over `helper.Float` (`Ma`, `MovingMax`, `MovingMin`, `MovingSum`, and `McGinleyDynamic` use the broader `helper.Number`).

## Testing Standard

Tests in this package follow the project's standard CSV-based testing pattern (as detailed in the root `AGENTS.md`), using `testdata/*.csv` files (e.g., `macd.csv`, `ema.csv`) for historical data validation.
